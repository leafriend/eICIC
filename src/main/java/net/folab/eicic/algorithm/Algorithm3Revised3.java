...
public class Algorithm3Revised3 extends Algorithm3 {
    ...
    public void chooseMobileConnection(Mobile[] mobiles) {
        ...
        for (Mobile mobile : mobiles) {
            ...
            double macroChannelCount = mobile.getActiveMacroChannelCount();
            double picoChannelCount = mobile.getActivePicoChannelCount();

            if (macroChannelCount > picoChannelCount) {
                // Macro로 할당받은 서브채널 갯수가 더 많은 경우
                mobileConnectsMacro[mobile.idx] = true;

            } else if (macroChannelCount < picoChannelCount) {
                // Pico로 할당받은 서브채널 갯수가 더 많은 경우
